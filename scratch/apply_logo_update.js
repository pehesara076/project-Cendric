...
// Backup
fs.writeFileSync(jsPath + '.bak', js, 'utf8');
...
if (!js.includes(heroOld) || !js.includes(mobileOld) || !js.includes(sidebarOld)) {
  console.error('Error: Could not find all targets in index JS');
  process.exit(1);
}

js = js.replace(heroOld, heroNew);
js = js.replace(mobileOld, mobileNew);
js = js.replace(sidebarOld, sidebarNew);

fs.writeFileSync(jsPath, js, 'utf8');
console.log('Successfully updated index-k68ZFBuM.js with new gradient C logo!');
